import { paletteNames, PaletteName } from './lib/theme';
...
<div className="relative grid grid-cols-7 gap-2.5 md:gap-1.5">
	{#each accentColorNames as colorName, i}
		...
		<button
			...
			className={`aspect-square min-h-5 w-full min-w-5 cursor-pointer rounded-md shadow-sm transition-all duration-150 ${isSelected ? 'scale-105' : 'opacity-80 hover:scale-110 hover:opacity-100'}`}
		>
...
<div className="mt-4 flex items-center">
	<label className="flex cursor-pointer items-center">
		...
		<span className="text-subtext0 ml-2 text-sm"
			>Background effect: <span className="text-accent">{$BackgroundEnabled ? 'on' : 'off'}</span></span
		>
	</label>
</div>
);
